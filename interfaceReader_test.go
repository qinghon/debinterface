...
`), 0644)
	...
	t.Log(time.Now().UnixNano() - now.UnixNano())
...
`), 0644)
	...
	for n := 0; n < b.N; n++ {
		// 被测试的功能
		//err := reader.ReadLines("/tmp/testInterface1")
		reader := NewReader("/tmp/testInterface1")
		reader.ReadLines("/tmp/testInterface1")
		//fmt.Printf("%d\n",n)
		//if err != nil {
		//	b.Error(err)
		//}
...
func BenchmarkInterfaceReader_ReadFromRD(b *testing.B) {
	ioutil.WriteFile("/tmp/testInterface1", []byte(`
# madwifi-ng WDS Bridge
#source /etc/network/interfaces.d/*
auto br0
...
`), 0644)
	...
	b.ReportAllocs()
	//b.N=1
	f, err := os.Open("/tmp/testInterface1")
	if err != nil {
		return
	}
	defer f.Close()
...
func BenchmarkInterfaceReader_ReadFromRD_2(b *testing.B) {
	buf := bytes.Buffer{}
	buf.WriteString(`
# madwifi-ng WDS Bridge
#source /etc/network/interfaces.d/*
